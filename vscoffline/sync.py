...
class VSCUpdates(object):
    ...
    @staticmethod
    def latest_versions(config):
        versions = {}
        for platform in config.platforms:
            for architecture in config.architectures:
                for buildtype in config.buildtypes:
                    for quality in config.qualities:
                        if quality == 'insider' and not config.checkinsider:
                            continue
...
if __name__ == '__main__':
    ...
    parser.add_argument('--platforms', dest='platforms',
                        default=vsc.PLATFORMS, help='Platforms to check')
    parser.add_argument('--architectures', dest='architectures',
                        default=vsc.ARCHITECTURES, help='Architectures to check')
    parser.add_argument('--buildtypes', dest='buildtypes',
                        default=vsc.BUILDTYPES, help='Build types to check')
    parser.add_argument('--qualities', dest='qualities',
                        default=vsc.QUALITIES, help='Qualities to check')
    ...
    # Splits the list of platforms, architecturs, build types and qualities
    #  if it was taken from the command line
    if isinstance(config.platforms, str):
        config.platforms = config.platforms.split(',')

    if isinstance(config.architectures, str):
        config.architectures = config.architectures.split(',')
    
    if isinstance(config.buildtypes, str):
        config.buildtypes = config.buildtypes.split(',')

    if isinstance(config.qualities, str):
        config.qualities = config.qualities.split(',')
    ...
    while True:
        ...
        if config.checkbinaries and not config.skipbinaries:
            ...
            versions = VSCUpdates.latest_versions(config)
